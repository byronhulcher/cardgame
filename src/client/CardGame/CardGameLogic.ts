...
export const CardGameLogic = kea({
  ...
  actions: (): CardGameActions => ({
    harvestResources: (player: Player, modifier: number) => ({
      player,
      modifier,
    }),
    resetGame: () => null,
  ...
  reducers: () => ({
    resourcePoints: [
      [
        0,
        0,
      ] as ResourcePointsTuple,
      {
        resetGame: () => [
          0,
          0,
        ] as ResourcePointsTuple,
        playCard: (currentResourcePoints: ResourcePointsTuple, {
          player,
          card,
    ...
    victoryPoints: [
      [
        0,
        0,
      ] as VictoryPointsTuple,
      {
        resetGame: () => [
          0,
          0,
        ] as VictoryPointsTuple,
        playCard: (currentVictoryPoints: VictoryPointsTuple, {
          player,
          card,
        }: { player: Player, card: Card }): VictoryPointsTuple =>
          getCardData(card).updateVictoryPoints(currentVictoryPoints, player),
        harvestResources: (currentVictoryPoints: VictoryPointsTuple, {
          player,
          modifier,
        }: { player: Player, modifier: number }) =>
    ...
    hands: [
      ...
      {
        playCard: (currentHandTuple: HandTuple, {
          player,
          card,
        }: { player: Player, card: Card }) =>
          currentHandTuple.map((hand, index) => index === player ? hand.filter((handCard) => handCard !== card) : hand),
        drawCard: (currentHandTuple: HandTuple, {
          player,
          card,
        }: { player: Player, card: Card }) =>
          currentHandTuple.map((hand, index) => index === player ? [
            ...hand,
            card,
          ] : hand),
      },
    ...
    decks: [
      ...
      { drawCard: (currentDeckTuple: DeckTuple, {
        player,
        card,
      }: { player: Player, card: Card }) =>
