...
const Points: React.FC<{ name: string, symbol: string, points: number, max: number }> = ({ name, symbol, points, max }) => (<h3>{name} Points: {Array(points).fill(symbol).join("")}{Array(Math.max(max - points, 0)).fill("⬛").join("")} </h3>)
const VictoryPoints: React.FC<{ points: number }> = (props) => (<Points name="Victory" symbol="🌞" {...props} max={10} />)
